...
export default function AskPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f2efe9] flex items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-2xl flex flex-col" style={{ height: "min(700px, 90vh)" }}>

        {/* Card */}
        <div className="flex-1 flex flex-col rounded-2xl overflow-hidden border border-black/[0.07] bg-white shadow-[0_4px_40px_rgba(0,0,0,0.08)]">

          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-black/[0.06] bg-white flex-shrink-0">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#111] to-[#333] flex items-center justify-center text-white text-sm font-medium tracking-tight">
                  H
                </div>
            ...
            <div className="flex items-center gap-1.5">
              <button className="w-8 h-8 rounded-lg border border-black/[0.08] flex items-center justify-center text-[#999] hover:text-[#333] hover:bg-[#f5f5f5] transition-colors">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto px-5 py-5 bg-[#faf9f7] flex flex-col gap-4">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-2.5 ${msg.type === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-medium flex-shrink-0 mt-0.5 ${
                  msg.type === "bot"
                    ...
                    : "bg-[#ebebeb] text-[#666] border border-black/[0.08]"
                }`}>
                  {msg.type === "bot" ? "H" : "U"}
                </div>

                <div className={`flex flex-col gap-1 max-w-[80%] ${msg.type === "user" ? "items-end" : ""}`}>
                  <div className={`px-4 py-2.5 text-[14px] leading-relaxed ${
                    msg.type === "user"
                      ? "bg-[#111] text-white rounded-2xl rounded-br-[4px]"
                      : "bg-white text-[#1a1a1a] rounded-2xl rounded-bl-[4px] border border-black/[0.07] shadow-[0_1px_4px_rgba(0,0,0,0.05)]"
                  }`}>
                    {msg.text}
                  </div>
                  <span className="text-[11px] text-[#bbb] px-1">{msg.time}</span>
                </div>
              </div>
            ))}
            ...
            {isTyping && (
              <div className="flex gap-2.5 items-start">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#111] to-[#333] flex items-center justify-center text-[11px] font-medium text-white flex-shrink-0 mt-0.5">H</div>
                <div className="bg-white border border-black/[0.07] shadow-[0_1px_4px_rgba(0,0,0,0.05)] rounded-2xl rounded-bl-[4px] px-4 py-3.5 flex items-center gap-1.5">
                  {[0, 180, 360].map((delay) => (
                    <span
                      key={delay}
          ...
          {/* Quick suggestions */}
          <div className="px-5 py-3 border-t border-black/[0.05] bg-white flex-shrink-0">
            ...
            <div className="flex gap-2 overflow-x-auto pb-0.5" style={{ scrollbarWidth: "none" }}>
              {quickQuestions.map((q, i) => (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-black/[0.10] bg-[#faf9f7] hover:bg-[#f0ede7] hover:border-black/[0.18] text-[13px] text-[#333] whitespace-nowrap flex-shrink-0 transition-all duration-150"
                >
          ...
          {/* Input */}
          <div className="px-4 pt-3 pb-2 border-t border-black/[0.05] bg-white flex-shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="flex-1 flex items-center gap-2 bg-[#f5f4f1] border border-black/[0.08] rounded-2xl px-4 py-2.5 focus-within:border-black/20 transition-colors">
